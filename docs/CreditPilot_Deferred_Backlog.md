...
4. Other customers categorical fields (scenario, risk_tags, payment_health membership questions) -- same shape as utilization/credit-rating, now largely covered by root cause #1's fix (data completeness), may not need a dedicated deterministic list on top given the demonstrated success of the credit-rating-threshold test.
